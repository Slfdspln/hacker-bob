...
function loadEgress(projectRoot) {
  return require(path.join(projectRoot, "mcp", "lib", "egress-profiles.js"));
}

function usage() {
  console.error(`Usage:
  node .claude/hooks/bob-egress.js [project-dir] list [--json]
  node .claude/hooks/bob-egress.js [project-dir] add <name> [--proxy-env ENV] [--proxy-url URL] [--region REGION] [--description TEXT] [--disabled] [--json]
  node .claude/hooks/bob-egress.js [project-dir] test <name> [--url URL] [--json]
  node .claude/hooks/bob-egress.js [project-dir] enable <name> [--json]
  node .claude/hooks/bob-egress.js [project-dir] disable <name> [--json]
  node .claude/hooks/bob-egress.js [project-dir] remove <name> --yes [--json]`);
}
...
function printResult(result, json) {
  ...
  if (Array.isArray(result.profiles)) {
    for (const profile of result.profiles) {
      ...
      process.stdout.write(`${profile.name}\t${enabled}\tregion=${region}\tproxy=${proxy}\t${description}\n`);
    }
    return;
  }
  ...
  if (result.profile) {
    ...
    process.stdout.write(`Profile: ${p.name} (${p.enabled ? "enabled" : "disabled"}), region=${p.region || "-"}, proxy=${p.proxy_configured ? "yes" : "no"}\n`);
  }
  if (result.observed) {
...
async function addProfile(egress, projectDir, args) {
  ...
  if (proxyUrl == null && process.stdin.isTTY) {
    const answer = await ask(`Proxy env var for ${name} (blank for ${envRefForProfile(name)}): `);
    proxyUrl = answer ? `\${${answer}}` : envRefForProfile(name);
  }
  if (proxyUrl == null) proxyUrl = envRefForProfile(name);
  if (region == null && process.stdin.isTTY) region = await ask("Region label (optional): ");
  if (description == null && process.stdin.isTTY) description = await ask("Description (optional): ");
  ...
  return {
    ...
    profile: egress.profilePublicView(egress.readEgressProfilesDocument
      ? egress.readEgressProfilesDocument(projectDir).profiles.find((profile) => profile.name === name)
      : egress.listEgressProfiles(projectDir).find((profile) => profile.name === name)),
  };
}
...
async function testProfile(egress, projectDir, args) {
  const name = firstPositional(args) || "default";
  const profile = egress.resolveEgressProfile(name, { projectRoot: projectDir });
  const agent = egress.createProxyAgent(profile.proxy_url);
  const testUrl = flagValue(args, "--url") || process.env.BOB_EGRESS_TEST_URL || "https://api.ipify.org?format=json";
...
async function main(argv) {
  ...
  const egress = loadEgress(parsed.projectDir);

  if (parsed.command === "list") {
    printResult({ profiles: egress.listEgressProfiles(parsed.projectDir) }, parsed.json);
    return;
  }

  if (parsed.command === "add") {
    printResult(await addProfile(egress, parsed.projectDir, parsed.args), parsed.json);
    return;
  }

  if (parsed.command === "test") {
    printResult(await testProfile(egress, parsed.projectDir, parsed.args), parsed.json);
    return;
  }
  ...
  if (parsed.command === "enable" || parsed.command === "disable") {
    ...
    printResult({ message: `${enabled ? "Enabled" : "Disabled"} egress profile "${name}".` }, parsed.json);
    return;
  }
  ...
  if (parsed.command === "remove") {
    ...
    printResult({ message: `Removed egress profile "${name}".` }, parsed.json);
    return;
  }
